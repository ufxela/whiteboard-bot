...
// returns number of bytes read in. 0 if no bytes (obviously)
int lex_readin_no_block(lex_t *l);

// read byte from <l>
int lex_getc(lex_t *l);

// doesn't block; -1 on no data
int lex_getc_no_block(lex_t *l);
